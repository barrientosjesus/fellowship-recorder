...
const shouldUpload = (cfg: ConfigService, metadata: Metadata) => {
  ...
  if (category === VideoCategory.Raids) {
    ...
    const uploadCurrentRaidOnly = flavour === Flavour.Retail &&
...
const getBaseConfig = (cfg: ConfigService): BaseConfig => {
  ...
  return {
    ...
    recordFellowship: cfg.get<boolean>("recordFellowship"),
    fellowshipLogPath: cfg.get<string>("fellowshipLogPath"),
  };
};
...
const validateBaseConfig = async (config: BaseConfig) => {
  ...
  if (recordFellowship) {
    const validPath = path.basename(fellowshipLogPath) === "CombatLogs";

    if (!validPath) {
      console.error("[Util] Invalid fellowship log path", fellowshipLogPath);
  ...
  if (recordEra) {
    ...
    const validPath = validFlavours.includes(getWowFlavour(eraLogPath)) &&
