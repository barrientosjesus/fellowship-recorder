...
import { ambiguate, isUnitPlayer, isUnitSelf } from "./logutils";
...
/**
 * Generic LogHandler class. Everything in this class must be valid for all
 * fellowship combat logs.
 *
...
export default abstract class LogHandler {
  ...
  constructor(logPath: string, dataTimeout: number) {
    ...
    this.combatLogWatcher.on("FELLOWSNIP_FORCE_STOP", () => {
  ...
  protected async handleEncounterStartLine(line: LogLine, flavour: Flavour) {
    ...
    if (LogHandler.activity) {
      console.warn("[LogHandler] Activity already in progress, skipping");
      return;
    }

    const startDate = line.date();
    const encounterID = parseInt(line.arg(1) as string, 10);
    const difficultyID = parseInt(line.arg(3) as string, 10);
  ...
  protected async handleEncounterEndLine(line: LogLine) {
    ...
    const result = Boolean(parseInt(line.arg(5) as string, 10));
  ...
  protected handleUnitDiedLine(line: LogLine): void {
    ...
    const playerGUID = line.arg(1) as string;
    const playerName = line.arg(2) as string;
    ...
    const isUnitUnconsciousAtDeath = Boolean(
      parseInt(line.arg(9) as string, 10),
    );
    ...
    const playerSpecId = LogHandler.activity.getCombatant(playerGUID)?.heroID ??
      0;
  ...
  protected static async endActivity() {
    ...
    const fellowshipRunning = poller.isFellowshipRunning();

    if (fellowshipRunning) {
      // Immediately queue the buffer start so it's ready if we go instantly into another activity.
      console.info(
        "[LogHandler] Queue buffer start as Fellowship still running",
      );
    ...
    try {
      ...
      const report = "Failed to stop recording, discarding: " +
    ...
    if (!videoFile) {
      ...
      const report = "No video file produced, discarding: " +
        lastActivity.getFileName();
      emitErrorReport(report);
  ...
  protected processCombatant(
    ...
    [combatant.name, combatant.realm, combatant.region] = ambiguate(
      srcNameRealm,
    );
  ...
  protected handleSpellDamage(line: LogLine) {
    ...
    const max = parseInt(line.arg(15) as string, 10);
    ...
    const current = parseInt(line.arg(14) as string, 10);
